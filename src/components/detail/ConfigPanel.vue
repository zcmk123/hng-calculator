...
<template>
    <aside class="cfg-col">
        ...
        <img v-if="weaponImg" :src="weaponImg" :alt="w.name" class="weapon-thumb" @error="onImgError" />
        <!-- ammo -->
        <div class="cfg-sel">
        ...
        <!-- mod blocks -->
        <div v-for="blk in modBlocks" :key="blk.catId" class="cfg-sel">
            <label>{{ t(MOD_CATEGORIES[blk.catId] || 'cfg.unknown') }}</label>
            <select :value="selectedModValue(blk.mods)" @change="onModChange(blk.mods, $event)">
                <option value="">{{ t('cfg.stock') }}</option>
                <option v-for="m in blk.mods" :key="m.id" :value="m.id">
                    {{ m.name }}
                </option>
            </select>
        </div>
        ...
        <div v-if="kind === 'explosive'" class="cfg-seg" :class="{ closed: !config.cfgOpen.grenadier }"
            data-key="grenadier">
...
.cfg-seg.closed label {
    color: var(--brass-dim);
}

.cfg-seg.closed :deep(.seg) {
    display: none;
}
